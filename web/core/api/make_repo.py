...
class Make_Repo(graphene.Mutation):
    ...
    def mutate(cls, root, info, team, name, description):
        try:
            ...
            (wq()
                .add_triple(svg_string,    '@schema:value', wq().string('SVG'))
                .add_triple(spec_icon_code_string, '@schema:value', wq().string(menu_button_svg))
                ...
                .add_triple(make_string,   '@schema:value', wq().string('Make'))
                .add_triple(any_string,    '@schema:value', wq().string('Any'))
                .add_triple(all_string,    '@schema:value', wq().string('All'))
                .add_triple(one_string,    '@schema:value', wq().string('One'))
                .add_triple(spec_string,   '@schema:value', wq().string('Spec'))
