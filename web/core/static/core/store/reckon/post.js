    ...
    node(d, n, cause=[]){ // make wrapper function to inser c and ax on every node reckon function ?!?!?!?!?! 
        try{
            ...
            paths.forEach((path, pi)=>{
                ...
                //total_length += length;
                var res = path.ribbon.length_v / input_ribbon_div; 
                for(let v=0; v<res; v++){
                ...
                code += '(Path '+pi+') \r\n';
                ribbon.get_point(0, 0, v1); // start point
                ribbon.get_point(1, 0, v2); v2.sub(v1); // start normal
                v3.copy(v1).add(v4.copy(v2).multiplyScalar(pulloff));
                if(offset_y == null) offset_y = -v1.y;
                move('G0', v3, {angles:axis_angles(v2)});
                move('G0', v1);
