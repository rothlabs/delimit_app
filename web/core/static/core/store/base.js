...
export const create_base_slice = (set,get)=>({
    ...
    send(d, patches){ // change to send patches directly to server (filtering for patches that matter)
        ...
        parts.forEach(n=>{ // don't update part that is being deleted on this send (not needed);    need to test with two profiles working on same asset
            if(d.n[n].t != 'profile'){
        ...
        ['p','b','i','f','s'].forEach(m=>{
            data[m].forEach(n=>{
                ...
                //if(d.node.be(d,n.id)){ // is this stopping undo delete from other client?!?!?!
                    ...
                    if(!d.n[n.id].r) d.n[n.id].r = {}; // should not clear this!!!
